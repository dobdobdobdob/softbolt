        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Space Mono', monospace" }}>
          ...
          <div style={{ position: "relative", background: "#fff", padding: "32px 24px", maxWidth: 380, width: "90%" }}>
            <div style={{ textAlign: "center", marginBottom: 6 }}>
              <svg width="20" height="28" viewBox="0 0 16 22" fill="none"><path d="M2 0 L14 0 L8 9 L13 9 L3 22 L6 12 L1 12 Z" fill="#CCFF00"/></svg>
            </div>
...
function ArtistDashboard({ onClose, onPreviewProfile }) {
  const d = DASH_DATA;
...
export default function MusicDiscoveryApp() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", position: "relative", fontFamily: "'Space Mono', 'Courier New', monospace", color: "#000" }}>
      ...
      {/* ── Artist Sign-up ── */}
      {artistPage && <ArtistPage artistName={artistPage} onClose={() => setArtistPage(null)} onPlay={playTrack} currentTrack={currentTrack} isPlaying={isPlaying} liked={liked} onLike={toggleLike} onArtistTap={setArtistPage} supported={supported} onSupport={(name, tier) => { setSupported(prev => { const next = new Map(prev); if (tier === null) { next.delete(name); } else { next.set(name, tier); } return next; }); }} likedArtists={likedArtists} onLikeArtist={name => setLikedArtists(prev => { const next = new Set(prev); next.has(name) ? next.delete(name) : next.add(name); return next; })} />}
      {artistSignup && <ArtistSignup onClose={() => setArtistSignup(false)} onOpenDashboard={() => { setArtistSignup(false); setArtistDashboard(true); }} />}
      {artistDashboard && <ArtistDashboard onClose={() => setArtistDashboard(false)} onPreviewProfile={(name) => { setArtistDashboard(false); setArtistPage(name); }} />}
